...
/**
 * @author :LiuQiang
 * @version ：2012-7-23 class desc
 */
public class ActUserCommentSend extends ActivityTemplate implements
		OnClickListener {
	...
	private String strRid = "", strUName = "", strTid = "";
	...
	private Runnable loadRunnable = new Runnable() {
		...
		@Override
		public void run() {
			arrList = bllGet.getCommentDialog(strMsgUid, strTid);
			if (arrList != null && arrList.size() > 0) {
				strRid = arrList.get(arrList.size() - 1).getId();
			}
	...
	private void send() {
		dlgProgress = ProgressDialog.show(ActUserCommentSend.this, "请等待...",
				"正在进行操作.....", true);
		dlgProgress.show();
	...
	private boolean netCheck() {
		...
		if (!strInput.equals("")) {
			...
			str = bllSend.comment(strTid, "", strMsgUid, strRid, strInput);
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 2:
				...
				Toast.makeText(ActUserCommentSend.this, strMessage,
						Toast.LENGTH_LONG).show();
				break;
			case 3:
				adapter = new MsgDialogAdapter(ActUserCommentSend.this, arrList);
				lvMsg.setAdapter(adapter);
				llLoad.setVisibility(View.GONE);
				if (arrList == null || arrList.size() < 1) {
					ActUserCommentSend.this.finish();
				}
				break;
			}
		}
